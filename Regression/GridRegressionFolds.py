...
def run_models(ms_info, list_of_models, ms_file_name, feature_reduce_choice):
    ...
    for name, model, param_grid in list_of_models:
        ...
        # Loop over the outer fold indices to perform outer CV
        for repeat_idx in range(10):
            r2_score_repeat = []  # Store R2 scores for each repeat
            for fold_idx in range(5):
                train_idx, test_idx = outer_fold_indices[repeat_idx * 5 + fold_idx]
            ...
            with open(os.path.join(current_working_dir, f'output_{name}/r2_scores_repeat_{repeat_idx + 1}.txt'), 'w') as f:
        ...
        overall_mean_R2 = np.mean(mean_R2)
        overall_std_R2 = np.sqrt(np.mean(std_R2 ** 2))
        ...
        with open(os.path.join(current_working_dir, f'output_{name}/metrics_{name}.txt'), 'w') as f:
            ...
            f.write(f'Overall Mean R2: {overall_mean_R2}\n')
            f.write(f'Overall Std R2: {overall_std_R2}\n')
